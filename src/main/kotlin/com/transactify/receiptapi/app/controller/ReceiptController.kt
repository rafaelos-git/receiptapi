...
class ReceiptController(private val receiptService: ReceiptService) {
    @PostMapping("/receipt")
    fun createReceipt(@RequestBody paymentDto: PaymentDto): ResponseEntity<Void> {
        logger().info("Creating receipt for payment: $paymentDto")
        receiptService.createReceipt(paymentDto)
